...
<div class="modal-header">
     <h6 class="modal-title" id="exampleModalLabel">Edit '{{ $role->name }}' Role</h6>
     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
     <form action="{{ route('admin.user-module.role.update', $role->id) }}" method="POST" class="ajax-form">
          @csrf
          ...
          <div class="row">
               ...
               <div class="col-md-12 form-group">
                    ...
                    <select name="is_active" class="form-control">
                         <option value="1" @if( $role->is_active ) selected @endif >Active</option>
                         <option value="0" @if( !$role->is_active ) selected @endif >In active</option>
                    </select>
               </div>
                         ...
                         <div class="col-12 col-md-3 my-2">
                              <div class="permission_block p-1">
                                   <label class="module-header mb-0 d-flex align-items-center">
                                        <input type="checkbox" class="module_check me-2" name="permissions[]"
                                             value="{{ $modulePermissionId }}"
                                             @if ($role->permissions->contains('id', $modulePermissionId))
                                        checked
                                        @endif
                                        />
                                   ...
                                   <ul class="sub_module_block mt-2 ps-3">
                                        ...
                                        <li class="sub-permission-item">
                                             <label class="mb-0 d-flex align-items-center">
                                                  <input type="checkbox" class="sub_module_check me-2" name="permissions[]" value="{{ $permission->id }}" @if ($role->permissions->contains('id', $permission->id)) checked @endif />
